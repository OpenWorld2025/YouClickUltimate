...
# -------------------------------------------------------------------
# Änderungen:
# 1. In der __init__-Methode wird geprüft, ob die Config-Datei existiert.
#    Falls nicht, wird sie sofort mit den Standardwerten geschrieben.
# 2. Falls ein Fehler beim Laden der Config auftritt, wird ein Fallback implementiert.
# 3. Die Video-Abdunklungs-Funktion wird so umprogrammiert, dass im Nachtmodus,
#    wenn die Checkbox angehakt ist, der Filter über ein GUI-Overlay angewendet wird.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # QSettings: Organisation und Anwendung; Config-Datei mit Endung .conf
        ...
        # Falls die Config-Datei nicht existiert, wird sie erstellt.
        if not os.path.exists(self.config_file):
            self.save_settings()
        try:
            self.settings = QSettings(self.config_file, QSettings.IniFormat)
        except Exception as e:
            logging.error("Fehler beim Laden der Konfigurationsdatei: " + str(e))
            self.settings = QSettings(self.config_file, QSettings.IniFormat)
            self.save_settings()
        self.log_messages = []
        ...
        self.current_theme = "day"  # "day" oder "night"
        ...
        # Dauerhaften LoadFinished-Handler registrieren, der im Nachtmodus den Filter anwendet
    ...
    def import_config(self):
        ...
        if src:
            try:
                ...
                self.load_settings()
                self.log_status("Config importiert: " + src)
            except Exception as e:
